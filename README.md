...
- [Buyer's / Renter's Agent Enhancement Task Plan](BUYERS_RENTERS_AGENT_TASK_PLAN.md)
...
# Preflight Domain connectivity before a report run.
python domain_connection_check.py --json

# If Akamai challenges the browser, open a normal Chromium window for clearance.
python domain_clearance.py
```

## Domain Data Backbone

Domain access is intentionally browser-shaped: headed Chromium, persistent local
profile, Australian locale/timezone, slow navigation, and no asset blocking when
headed. This keeps the hunter close to a polite human checking properties rather
than a brittle API dependency.

Run `python domain_connection_check.py --json` before relying on a full live
folio run. `status: blocked` means Domain/Akamai challenged the browser before
the page exposed `digitalData` or `__NEXT_DATA__`.

If the check is blocked, run `python domain_clearance.py`, complete any visible
Domain prompt in the normal Chromium window, confirm listings load, close the
window, then run the connection check again.
